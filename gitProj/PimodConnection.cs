	...
	internal class PimodOdbcConnection : PimodConnection
	{
		...
		internal static string CreateOdbcConnectionString(PimodConnectionInfo connectionInfo)
		{
			...
			if (String.IsNullOrEmpty(connectionInfo.NativeDriver))
			{
				...
				//
				connectionBuilder.Driver = "ODBC Driver 19 for SQL Server";
			}
			else
			{
		...
		internal static string StripOdbcDriverHeader(string message)
		{
			...
			// "[Microsoft][ODBC Driver 19 for SQL Server][SQL Server]"
			// "ERROR [22012] [Microsoft][ODBC Driver 19 for SQL Server][SQL Server]"
			// "ERROR [42000] [Microsoft][ODBC Driver 19 for SQL Server][SQL Server]"
			//
			Regex odbcHeader = new Regex(@"^(ERROR \[[\d\w]*\])?\s*\[Microsoft\](\[SQL Server Native Client \d+\.\d+\]|\[ODBC Driver 19 for SQL Server\])(\[SQL Server\])?", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Multiline);
