...
namespace Microsoft.SqlServer.Test.Pimod
{
	...
	internal class PimodOdbcConnection : PimodConnection
	{
		...
		internal static string StripOdbcDriverHeader(string message)
		{
			...
			//
			Regex odbcHeader = new Regex(@"^(ERROR \[[\d\w]*\])?\s*\[Microsoft\](\[SQL Server Native Client \d+\.\d+\]|\[ODBC Driver \d+ for SQL Server\])(\[SQL Server\])?", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Multiline);
